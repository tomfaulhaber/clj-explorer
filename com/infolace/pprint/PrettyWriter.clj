...
(ns com.infolace.pprint.PrettyWriter
  (:use com.infolace.pprint.utilities)
  (:gen-class
   :extends com.infolace.pprint.ColumnWriter
   :init init
...
(defmethod write-token :indent [#^com.infolace.pprint.PrettyWriter this token]
  (let [lb (:logical-block token)]
    (ref-set (:indent lb) 
             (+ (:offset token)
...
(defmethod write-token :buffer-blob [#^com.infolace.pprint.PrettyWriter this token]
  (.col-write this #^String (:data token)))

(defmethod write-token :nl [#^com.infolace.pprint.PrettyWriter this token]
...
(defn- write-tokens [#^com.infolace.pprint.PrettyWriter this tokens force-trailing-whitespace]
  (doseq [token tokens]
...
(defn- tokens-fit? [#^com.infolace.pprint.PrettyWriter this tokens]
...
(defn- miser-nl? [#^com.infolace.pprint.PrettyWriter this lb section]
...
(defn emit-nl [#^com.infolace.pprint.PrettyWriter this nl]
  (.col-write this (int \newline))
  (dosync (setf :trailing-white-space nil))
...
(defn- write-line [#^com.infolace.pprint.PrettyWriter this]
  (dosync
...
;;; writing
(defn- add-to-buffer [#^com.infolace.pprint.PrettyWriter this token]
;  (prlabel a2b token)
  (dosync
...
(defn- write-buffered-output [#^com.infolace.pprint.PrettyWriter this]
  (write-line this)
...
(defn- write-initial-lines 
  [#^com.infolace.pprint.PrettyWriter this #^String s] 
  (let [lines (.split s "\n" -1)]
    (if (= (count lines) 1)
      s
      (dosync 
...
(defn write-white-space [#^com.infolace.pprint.PrettyWriter this]
  (if-let [#^String tws (getf :trailing-white-space)]
    (dosync
     (.col-write this tws)
     (setf :trailing-white-space nil))))
...
(defn- -write 
  ([#^com.infolace.pprint.PrettyWriter this x]
...
(defn- write-char [#^com.infolace.pprint.PrettyWriter this #^Integer c]
...
(defn- -flush [#^com.infolace.pprint.PrettyWriter this]
...
(defn- -newline [#^com.infolace.pprint.PrettyWriter this type]
  (dosync 
   (setf :mode :buffering)
   (add-to-buffer this (make-nl type (getf :logical-blocks)))))

(defn- -indent [#^com.infolace.pprint.PrettyWriter this relative-to offset]
  (dosync 
   (let [lb (getf :logical-blocks)]
     (if (= (getf :mode) :writing)
       (do
...
(defn- -getMiserWidth [#^com.infolace.pprint.PrettyWriter this]
  (getf :miser-width))
